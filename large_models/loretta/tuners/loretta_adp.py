...
class LorettaAdpModel(torch.nn.Module):
    ...
    def _find_and_replace(self):
        ...
        for key in key_list:
            ...
            if target_module_found:
                ...
                if adapter_type == "conv_adapter":
                    nx, nf = target.weight.shape
                    new_module = Linear(nx, nf, tensorized=self.peft_config.tensorized,
                                        tensor_rank=self.peft_config.tensor_rank, bias=bias, **kwargs)
    ...
    def _replace_module_conv(self, parent_module, child_name, new_module, old_module):
        setattr(parent_module, child_name, new_module)
        new_module.weight.data = old_module.weight.data.squeeze(-1)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if tensorized:
            ...
            self.adapter_up = wrapped_linear_layers(in_features=bottleneck_size, out_features=in_features, tensorized=True, config=config_tensor_up)
        else:
    ...
    def forward(self, x: torch.Tensor):
        if self.disable_adapters:
            ...
            if self.adapter_type == "mh_adapter":
                ...
                x = x.view(size_out)
                print(f'x shape {x.shape} size_out {size_out}')
                expected_dtype = x.dtype
                residual = x
                ...
                output = self.adapter_up(self.act_fn(self.adapter_down(self.adapter_dropout(x)))).to(
                    expected_dtype) * self.adapter_scaling
                print(f' output {output.shape} residual {residual.shape}')
                result = output + residual
...
class GPTLinear(GPT2MLP, AdapterLayer):
    """
    Bottleneck adapter in a dense layer. The adapter can be applied after the multi-head attention layer and/or
    after the feed-forward layer.
    """
    ...
    def __init__(
        ...
        GPT2MLP.__init__(self, in_features, out_features, **kwargs)
        AdapterLayer.__init__(self, bottleneck_size=bottleneck_size,
                              non_linearity=non_linearity,
                              adapter_dropout=adapter_dropout,
                              scaling=scaling)
        ...
        config_tensor_up = config_class(shape=tensor_shape_up, ranks=tensor_rank, set_scale_factors=False)
        if tensorized:
            self.adapter_down = wrapped_linear_layers(in_features=in_features, out_features=bottleneck_size,
                                                      tensorized=True, config=config_tensor_down)
            self.adapter_up = wrapped_linear_layers(in_features=bottleneck_size, out_features=in_features,
                                                    tensorized=True, config=config_tensor_up)
        else:
            self.adapter_down = nn.Linear(in_features, bottleneck_size, bias=False)
            self.adapter_up = nn.Linear(bottleneck_size, out_features, bias=False)
        self.act_fn = ACT2FN[self.non_linearity]
