...
import wandb
import numpy as np
# from trainer_admm import OurTrainer as AdmmTrainer
...
from trainer import OurTrainer as Trainer
...
def main():
    ...
    wandb_run_name = str(data_args.task_name) + '-' + 'roberta-large' + '-' \
                     + str(training_args.learning_rate) + str(training_args.tuning_type.replace('_', '-')) + '-lorar-' + str(training_args.lora_r) \
                     + '-rate-' + str(training_args.inc_rate)+ '-sch-' + str(training_args.lr_scheduler_type.replace('.', '-'))
    print(f'wandb name {wandb_run_name}')
    wandb.init(project="<zeta-roberta>", name=wandb_run_name)
    ...
    config_kwargs = {'apply_lora': model_args.apply_lora,
                    ...
                    'lora_r': training_args.lora_r,
                     "tuning_type": training_args.tuning_type,
                     'tensor_rank': training_args.tensor_rank,
    }
    if model_args.apply_lora:
        if 'roberta' in model_args.model_name_or_path:
            print(f'use the designed config')
    ...
    # path = os.path.join('/global/cfs/cdirs/m4645/yifanycc/data', 'TASK-' + str(data_args.task_name) + '-K-' + str(training_args.num_k)  + 'train_dataset.pth')
    # print(f'data dir {path}')
    # torch.save(train_dataset, os.path.join('/global/cfs/cdirs/m4645/yifanycc/data', 'TASK-' + str(data_args.task_name) + '-K-' + str(training_args.num_k)  + 'train_dataset.pth'))
    #
    # torch.save(eval_dataset, os.path.join('/global/cfs/cdirs/m4645/yifanycc/data', 'TASK-' + str(data_args.task_name) + '-K-' + str(training_args.num_k)  + 'eval_dataset.pth'))
    # torch.save(test_dataset, os.path.join('/global/cfs/cdirs/m4645/yifanycc/data', 'TASK-' + str(data_args.task_name) + '-K-' + str(training_args.num_k)  + 'test_dataset.pth'))
    #
    # quit()
    # train_dataset = torch.load(os.path.join('/global/cfs/cdirs/m4645/yifanycc/data', 'TASK-' + str(data_args.task_name) + '-K-' + str(training_args.num_k)  + 'train_dataset.pth'))
    # eval_dataset = torch.load(os.path.join('/global/cfs/cdirs/m4645/yifanycc/data', 'TASK-' + str(data_args.task_name) + '-K-' + str(training_args.num_k)  + 'eval_dataset.pth'))
    # test_dataset = torch.load(os.path.join('/global/cfs/cdirs/m4645/yifanycc/data', 'TASK-' + str(data_args.task_name) + '-K-' + str(training_args.num_k)  + 'test_dataset.pth'))
    set_seed(training_args.seed)
    ...
    # wrap PEFT adapter
    # if training_args.tuning_type == 'loretta_rep':
    #     from loretta import LorettaRepConfig, get_peft_model, TaskType
    #     peft_config = LorettaRepConfig(
    #         r=training_args.rep_bottleneck,
    #         lora_alpha=training_args.rep_alpha,
    ...
    if training_args.tuning_type == 'adazeta':
        from adapters import add_roberta_adapters, AdapterConfig, freeze_all_parameters, unfreeze_roberta_adapters
        adapter_config = AdapterConfig(
            hidden_size=1024, adapter_size=64,
            adapter_act='relu', adapter_initializer_range=1e-2, tensorized=True, tensor_rank=training_args.tensor_rank,
        ...
        model.roberta = unfreeze_roberta_adapters(model.roberta, True)
        model.classifier.requires_grad = False
    if training_args.tuning_type != 'lora':
        for name, param in model.named_parameters():
            if "lm_head" in name:
                param.requires_grad = False
        if training_args.tuning_type == 'loretta_rep' or training_args.tuning_type == 'loretta_adp':
            for name, param in model.named_parameters():
