...
from loretta.tuners.loretta_rep import Linear as LorettaRepLinear
from loretta.tuners.loretta_adp import Linear as LorettaAdpLinear
...
class RobertaConfig(BertConfig):
    ...
    def __init__(self, pad_token_id=1, bos_token_id=0, eos_token_id=2, apply_lora=False, lora_r=None, lora_alpha=None, tuning_type=None, tensor_rank=None, **kwargs):
...
class RobertaSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        if getattr(config, "apply_lora", False):
            if config.tuning_type == 'lora':
                self.query = lora.Linear(config.hidden_size, self.all_head_size, config.lora_r, lora_alpha=config.lora_alpha)
            elif config.tuning_type == 'loretta_rep':
                self.query = LorettaRepLinear(config.hidden_size, self.all_head_size, config.lora_r, lora_alpha=config.lora_alpha, tensor_rank=config.tensor_rank)
            else:
        ...
        if getattr(config, "apply_lora", False):
            if config.tuning_type == 'lora':
                ...
                self.value = LorettaRepLinear(config.hidden_size, self.all_head_size, config.lora_r, lora_alpha=config.lora_alpha, tensor_rank=config.tensor_rank)
            else:
