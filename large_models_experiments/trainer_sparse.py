...
class OurTrainer(Trainer):
    ...
    def __init__(self, evaluate_func, eval_samples, *args, **kwargs):
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                #            "step_consumption": train_step_duration * 1000})
                # print(f'check eval steps {self.args.eval_steps} total steps {total_steps} now {(total_steps + 1) % self.args.eval_steps}')
    ...
    def zo_perturb_parameters(self, random_seed=None, scaling_factor=1):
        ...
        for name, param in self.named_parameters_to_optim:
            ...
            random_tensor = torch.rand(size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
            mask = (random_tensor > sparsity).float()
            # print(f'check mask total {mask.numel()} has value {mask.sum()}')
            z.mul_(mask)
